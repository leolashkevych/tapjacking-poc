package com.b0ss.tapjacking_poc;
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        float scaleW = (float) 0.9;
        float scaleH = (float) 0.7;

        DisplayMetrics metrics = new DisplayMetrics();

        getWindowManager().getDefaultDisplay().getMetrics(metrics);

        hS = (int) (metrics.heightPixels *scaleH);
        wS = (int) (metrics.widthPixels *scaleW);



        Bitmap myBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.overlay10f);
        scaledBitmap = Bitmap.createScaledBitmap(myBitmap, wS, hS, true);


        // get the dimensions of the tapjacking image
        bd = new BitmapDrawable(scaledBitmap);
    ...
    public void startTJService(View view) {
        ...
        service.putExtra("hS",hS);
        service.putExtra("wS",wS);
